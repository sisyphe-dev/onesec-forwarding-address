...
export class WaitForForwardingTxStep extends BaseStep {
    ...
    details(): Details {
        return {
            ...
            description: "Wait for transaction that moves tokens from the forwarding address to the bridge",
        };
    }
    ...
    async run(): Promise<StepStatus> {
        this._status = {
            Pending: {
                ...
                description: "Waiting for transaction that moves tokens from the forwarding address to the bridge",
            },
        };

        const forwardingAddress = this.computeForwardingAddressStep.getForwardingAddress();
        const lastTransferId = this.computeForwardingAddressStep.getLastTransferId();
        ...
        try {
            ...
            const response = await this.onesec.getForwardingStatus(this.token, this.evmChain, forwardingAddress, this.icpAccount);

            const transferId = response.done;
            const status = response.status;

            if (lastTransferId === undefined || transferId && transferId > lastTransferId) {
                ...
                if ("Forwarded" in status) {
                    ...
                    this._status = {
                        Pending: {
                            ...
                            description: "OneSec is checking the balance of the forwarding address",
                        }
                    };
                    ...
                    this._status = {
                        Pending: {
                            ...
                            description: "OneSec is moving tokens from the forwarding address to the bridge",
                        }
                    };
